...
class CSVReader {
  async iterateCSV({ filePath, chunkSize, handler = ({ rows }) => {} }) {
    if (!filePath) {
      throw new CSVReaderFilePathInvalidError('file path should be a non-empty string')
    }

    if (chunkSize < 1) {
      throw new CSVReaderChunkSizeInvalidError('chunk size should be greater than 1')
    }
